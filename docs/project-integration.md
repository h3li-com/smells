...
- Both source modes emit report schema v3 with the same metric schema, input/implementation digests, repository-relative `implementation_results`, 23 canonically ordered repository `smell_results`, 23-item coverage inventory and sorted findings. Hooks should route by implementation and then smell-level state before following `matched_finding_indices` to exact rule evidence and 1-based line/UTF-8-character-column locations. Unavailable rules have explicit coverage status; no semantic smell is labeled absent from a structural pass.
