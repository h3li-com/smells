...
- The policy selects `typescript-v1` and `authored_source`. Full scans include `.ts`, `.tsx`, `.mts`, and `.cts`; staged scans read both policy and source from regular Git-index blobs.
- The locked Tree-sitter TypeScript grammar parses `.ts`, `.mts`, and `.cts`; the locked TSX grammar parses `.tsx`. Any syntax error makes the scan incomplete. No import execution, module loading, TypeScript compiler, test runner, or application code is run.
